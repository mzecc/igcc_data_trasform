...
def process_headers(file_path:Path, general:dict, specific:dict, ref_year: int)->xr.DataArray:
    ...
    df_gmst = pd.read_csv(
        ...
        skip_blank_lines=True,
        na_values=[" ", "  ", "\t"],
    ).drop(columns=["notes"], errors="ignore").loc[:, lambda df: ~df.columns.str.contains("^Unnamed")] # Drop empty name columns

    df_gmst_annual = df_gmst.iloc[0:2,:]
    df_gmst_sr15 = df_gmst.iloc[2:4,:]
    df_gmst_ar6 = df_gmst.iloc[4:,:]
    df_list = [
    ...
    if name == "Gillett_GMST_headlines":
        df_gsat = pd.read_csv(
            ...
            skip_blank_lines=True,
            na_values=[" ", "  ", "\t"],
        ).drop(columns=["notes"], errors="ignore").loc[:, lambda df: ~df.columns.str.contains("^Unnamed")]
        
        df_gsat_ar6 = df_gsat.iloc[0:2,:]
        df_gsat_sr15 = df_gsat.iloc[2:,:]
    ...
    for temp, definition, df in df_list:
        ...
        for p in percentiles:
            ...
            ds_tmp = ds_tmp.assign_coords(
                time=("time", df_time_in_days["time"].squeeze()),
            )
    ...
    # ds_gmst = xr.concat(gmst_nc, dim="definition", join="outer")
    ...
    # Attributes 
    ds["component"].attrs.update({'long_name':'Temperature attributed component', 'coverage_content_type': 'coordinate',
                                         'comment':'Attributed component of temperature change.'})
    ds["definition"].attrs.update({'long_name':'Definition calculation', 'coverage_content_type': 'coordinate',
                                      'comment':'Definition followed for the calculation.'})
    ds["percentile"].attrs.update({'long_name':'percentile', 'units': "1", 'coverage_content_type': 'coordinate',
                                   'comment':'Percentiles of the temperature distribution.'})
    ds["time"].attrs.update(general["dimensions"]["time"])
...
for file, metadata in specific.get("files", {}).items():
    ...
    df.rename(columns={"F-gases":"F_gases","CO2-LULUCF":"CO2_LULUCF","CO2-FFI":"CO2_FFI"},inplace=True)		
    ...
    elif name in ['Gillett_GMST_headlines','Ribes_GMST_headlines']:
...
df_gmst = pd.read_csv(
    file_path.parent / "Gillett_GMST_headlines.csv",
    engine="python",
    comment="#",
    skip_blank_lines=True,
).drop(columns=["notes"])
...
for temp, definition, df in df_list:
    ...
    df_time_in_days = year2dates(time_df, REF_YEAR)
    t_bounds_list.append(df_time_in_days)
    # t_bounds_list.append(df_time_in_days[["timebound_lower", "timebound_upper"]])#.to_numpy().tolist())
    # t_bounds_list.extend(df_time_in_days[["timebound_lower", "timebound_upper"]].values)
    ...
    for p in percentiles:
        ...
        # ds_tmp["time"].attrs.update(general["dimensions"]["time"])
        # ds_tmp["time_bnds"] = (("time", "nbnds"), df_time_in_days[["timebound_lower", "timebound_upper"]].values)

        # ds_tmp = ds_tmp.expand_dims( definition=[definition] )
        datasets.append(ds_tmp)
...
ds_gsat = xr.concat(gsat_nc, dim="definition", join="outer")
ds_gmst = xr.concat(gmst_nc, dim="definition", join="outer")
ds = xr.merge([ds_gsat, ds_gmst], compat="no_conflicts", join="outer")
# Attributes 
ds["component"].attrs.update({'long_name':'Temperature attributed component', 'coverage_content_type': 'coordinate',
                                     'comment':'Attributed component of temperature change.'})
ds["definition"].attrs.update({'long_name':'Definition calculation', 'coverage_content_type': 'coordinate',
...
bounds = pd.concat(t_bounds_list, ignore_index=True).drop_duplicates()
ds["time_bnds"] = (("time", "nbnds"), bounds.set_index("time").reindex(ds["time"])[["timebound_lower", "timebound_upper"]])#np.unique(np.array(t_bounds_list),axis=0))
# ds_tmp["time_bnds"] = (("time", "nbnds"), df_time_in_days[["timebound_lower", "timebound_upper"]])
